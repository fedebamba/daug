...
if __name__ == "__main__":
    ...
    if utils.checkconf(conf_file, "balanced", "bbb")[1] == "u" and utils.checkconf(conf_file, "balanced", "bbb")[2] == "b":
        train_set_length = int(total_train_data - (utils.checkconf(conf_file, "el_for_validation", 200)*10)) # todo: modificare
    el_for_validation = utils.checkconf(conf_file, "el_for_validation", 200) # todo: modificare
    ...
    class CifarLoader():
        def __init__(self, transform=None, first_time_multiplier=1, name=None, unbal=True, test_transform=None, selection_transform=None):
            ...
            self._test_set = customcifar.UnbalancedCIFAR100(root="./cifar", train=False, download=True, transform=test_transform, full_classes=self._train_val_set.full_classes, unbal_test=(not balanced_test_set))  # 10000
            ...
            print([len([x for x in self.train_indices if x in self._train_val_set.el_for_class[i] ]) for i in range(100)])
    ...
    def a_single_experiment(esname, esnumber, seed):
        ...
        best_net, best_acc = single_train_batch(num_of_epochs=epochs_first_step, dataset=dataset,
                                                ...
                                                test_distro=[1 if x in dataset._train_val_set.full_classes else difficult_classes_percentage for x in range(100)])
